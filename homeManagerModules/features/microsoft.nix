...
with lib;
let
  cfg = config.myHomeManager.microsoft;

  # Microsoft workspace launcher script
  microsoft-script = pkgs.writeShellScriptBin "microsoft" ''
    set -euo pipefail
  ...
  # Desktop entry for Microsoft Work Environment
  microsoft-work-desktop = pkgs.makeDesktopItem {
    name = "microsoft-edge-work";
    desktopName = "Edge Work Environment";
    comment = "Launch complete Microsoft work environment (Teams, Outlook, Admin browser)";
    icon = "microsoft-edge";
    exec = "${microsoft-script}/bin/microsoft --all";
    terminal = false;
    categories = [ "Network" "WebBrowser" "Office" ];
    keywords = [ "Teams" "Outlook" "Edge" "Microsoft" "Work" ];
    startupNotify = true;
  };
...
in {
  config = mkIf cfg.enable {
    home.packages = with pkgs; [
      ...
      # Desktop entry for launching work environment
      microsoft-work-desktop
